...
import ColorBarSelector from './ColorBarSelector';
import BottomRightToolbar from './BottomRightToolbar';
import AdditionalActionsToolbar from './AdditionalActionsToolbar';
// Theme
import { ThemeContext } from '@/context/ThemeContext';
// Saving functionality
import { saveAs } from 'file-saver';
import { saveDrawings } from '@/utils/saveDrawings';
...
function Canvas({sendMessage, setRoomID, incomingDrawings, roomID, usersList, username, loading}) {
    ...
    }, [cursorX, cursorY, leftMouseDown, prevCursorX, prevCursorY])
    ...
    }, [drawLine, incomingDrawings, toScreenX, toScreenY, username])
    ...
    useEffect(() => {
        if(!drawingHistory) return;
            ...
            // If firstIndex and lastIndex are different from -1, it means that there was a last object drawn
            // Iterate over the current drawings array
            // Save the elements of the array with indexes between firstIndex and lastIndex
            for(let i=0; i<drawings.length; i++)
                if(firstIndex >= 0 && lastIndex >= 0)
                    if(i > firstIndex && i < lastIndex)
                        newDrawings.push(drawings[i]);
            ...
            if(newDrawings.length)
                saveDrawings(roomID, newDrawings);
        }
    }, [drawingHistory, drawings, roomID])
    return (
        <div
        ...
        >
            ...
            <ConnectedUsers usersList={usersList}/>
            {showFileUploader
                && <FileUploader
